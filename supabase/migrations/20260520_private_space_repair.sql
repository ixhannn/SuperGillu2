-- Update the storage repair function to scan `private_space_items` and allow `data:` inline references.
...
as $$
  with refs as (
    ...
    -- COVERAGE: private_space_items storagePath
    -- Strip inline blobs (image/video/audio) from row_data to keep the RPC response small.
    select
      m.id::text as row_id,
      m.user_id,
      m.couple_id,
      'private_space_items'::text as source_table,
      m.data->>'id' as logical_row_id,
      'private-space'::text as feature,
      'storagePath'::text as field_name,
      coalesce(m.data->>'storagePath', m.data->>'image') as storage_path,
      coalesce(
        case when nullif(m.data->>'ownerUserId', '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          then nullif(m.data->>'ownerUserId', '')::uuid
          else null
      ...
      coalesce(nullif(m.data->>'date', ''), nullif(m.data->>'createdAt', '')) as item_timestamp,
      nullif(m.data->>'expiresAt', '')::timestamptz as expires_at,
      (m.data - 'image' - 'video' - 'audio') as row_data
    from public.private_space_items m

    union all

    -- COVERAGE: private_space_items videoStoragePath
    select
      m.id::text as row_id,
      m.user_id,
      m.couple_id,
      ...
      m.data->>'id' as logical_row_id,
      'private-space'::text as feature,
      'videoStoragePath'::text as field_name,
      coalesce(m.data->>'videoStoragePath', m.data->>'video') as storage_path,
      coalesce(
        case when nullif(m.data->>'ownerUserId', '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          then nullif(m.data->>'ownerUserId', '')::uuid
          else null
    ...
    -- COVERAGE: private_space_items audioStoragePath
    select
      m.id::text as row_id,
      m.user_id,
      m.couple_id,
      ...
      m.data->>'id' as logical_row_id,
      'private-space'::text as feature,
      'audioStoragePath'::text as field_name,
      coalesce(m.data->>'audioStoragePath', m.data->>'audio') as storage_path,
      coalesce(
        case when nullif(m.data->>'ownerUserId', '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          then nullif(m.data->>'ownerUserId', '')::uuid
          else null
