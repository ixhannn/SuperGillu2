...
- [x] **PostHog** (product analytics) — loaded via the official env-gated loader in `services/analytics.ts` with autocapture / pageviews / session-replay all OFF; only the 6 named events are sent, anonymous (so it also captures the pre-sign-in funnel the first-party sink can't).
