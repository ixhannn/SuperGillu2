...
**Theme-boot residual (Tier 2, deferred — needs CSP work):** the *pre-bundle* static splash (`index.html` hardcoded pink on `html,body`) still shows for the cold-load window before the JS bundle executes, so a **cold** launch on the dark `starry-night` theme briefly shows pink → dark. Eliminating it needs an inline `<head>` boot script, which the strict prod CSP (`script-src 'self'`) blocks — so it requires either a CSP hash or an external `/theme-boot.js`. Flagged, not applied (Tier 1 already fixes the React-loader crossfade for every theme on warm + cold).
