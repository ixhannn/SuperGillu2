...
assert.match(
  appSource,
  /const T_KEEP_ALIVE_TAB\s*=\s*240[\s\S]*setCurrentView\(destination\);[\s\S]*transitionLockRef\.current = false;[\s\S]*window\.setTimeout\(\(\) => \{[\s\S]*finalizeNavigation\(\);[\s\S]*\}, T_KEEP_ALIVE_TAB\)/,
  'Fast tab transitions should release the navigation lock immediately while keeping diagnostics aligned with the animation duration.',
);
