...
// merge on EVERY call — a measurable main-thread cost on the hot path.
//
// The cache key is the exact source strings, so it is fully self-invalidating:
// any write to identity/shared/lock changes a string → cache miss → recompute.
// No explicit invalidation needed, zero staleness risk. We hand back a shallow
// clone so callers that reassign a top-level field don't corrupt the cached
// object.
